...
//             300,0);
    ...
    public void FlipV() {
        ...
        for (int sx = 0; sx < p.getWidth(); sx++) {
            for (int sy = 0; sy < p.getHeight(); sy++) {

                sourcePixel = p.getPixel(sx, sy);
                Tx = sx;
                Ty = p.getHeight() - 1 - sy;
                targetPixel = this.getPixel(Tx, Ty);
                targetPixel.setColor(sourcePixel.getColor());
    ...
    public Picture changeColor(double r, double g, double b) {
    ...
    public void computeHistogram() {
        ...
        for (int x = 1; x < width; x++) {
            for (int y = 1; y < height; y++) {
                //int valueBefore=this.getRaster().getPixel(x, y,iarray)[0];
...
//                    iarray[0]=valueAfter;
//                     this.getRaster().setPixel(x, y, iarray); 
//                }
//            }
            }
        }
    ...
    public Picture crop(int sx, int sy, int endX, int endY) {
        Picture target = new Picture(this.getWidth(), this.getHeight());
        Pixel sourcePixel = null;
        Pixel targetPixel = null;

        for (int sourceX = sx, targetX = 0; sourceX < endX; sourceX++, targetX++) {
            // loop through the rows
            for (int sourceY = sy, targetY = 0; sourceY < endY; sourceY++, targetY++) {
                sourcePixel = this.getPixel(sourceX, sourceY);
    ...
    public int[] computeHistogram2() {
        ...
        for (int k = 0; k < this.getHeight(); k++) {
            ...
            for (int j = 0; j < this.getWidth(); j++) {
                ...
                ip = this.getPixel(j, k).getAverage();
                i = (int) ip;
    ...
    public double ComputeContrast() {
        int[] histogramArray = this.computeHistogram3();
        int max = 0;
        int min = 1;

        for (int i = 0; i < histogramArray.length; i++) {
            if (histogramArray[i] > max) {
                max = histogramArray[i];
            }
        }

        for (int j = 0; j < histogramArray.length; j++) {
            if (histogramArray[j] < min && histogramArray[j] != 0) {
                min = histogramArray[j];
            }
    ...
    //Brighteness of gray scale image is the average intensity of all pixels in image 
    public double computingBrighteness() {
        //First We Should Convert Picture To Gray Scale To Comput the Brighteness
        this.grayscale();
        Pixel pixel = null;
        Pixel[] pixelArray = this.getPixels();
        double brighteness;
        double averageIntensityOfAllPixel = 0.0;
        ...
        for (int y = 0; y < this.getHeight(); y++) {
            ...
            for (int x = 0; x < this.getWidth(); x++) {
                ...
                // get the gray scale averages for the  pixels
                averageIntensityOfAllPixel += pixel.getAverage();

            }
        }
        return (brighteness = averageIntensityOfAllPixel / pixelArray.length);
    }
